...
def database_access(df: pd.DataFrame, project_name: str, plm_model: str = "esm1b"):
    ...
    if (
    ...
    ):  # todo: recreate the collection instead and make user aware (press [Yn] or dont know yet to report)
        ...
        raise ValueError(
            f"Length of dataframe ({df.shape[0]}) and embeddings ({embeddings.shape[0]}) do not match. As a consequence, the collection is deleted and you need to embed again. So just re-run."
        )
...
def create_vector_db_collection(qdrant, df, embeddings, collection_name: str) -> list:
    ...
    annotation = df.iloc[
        ...
        orient="index"
    )  # only use 'accession' as key (0:1)  # (0, {'accession': 'Q9NWT6', 'Reviewed': True})
    logging.info("Creating Qdrant records. This may take a while.")
    for i, anno in tqdm(annotation.items()):
        vector = embeddings[i].tolist()
        record = models.Record(
            id=i, vector=vector, payload=anno
        )  # {'accession': 'Q9NWT6', 'Reviewed': True}
...
def load_or_create_db(
    ...
    if collection_name not in collection_names:
        ...
        annotation, embeddings = create_vector_db_collection(
            qdrant, df, embeddings, collection_name=collection_name
        )
    else:
