...
class MinimumSpanningTree:
    ...
    def plot_mst_in_dimred_landscape(self):
        ...
        for (x1, y1), (x2, y2) in zip(line_coords[:, 0], line_coords[:, 1]):
            edge_x.extend(
                [x1, x2, None]
            )  # None: A separator indicating the end of this edge
    ...
    def create_node_trace(
        self, node_x: np.ndarray, node_y: np.ndarray, node_adjacencies: np.ndarray
    ):
        """
        ...
        hover_text = [
            "<br>".join(f"{col}: {self.df[col][i]}" for col in columns_of_interest)
            + f"<br>connectivity: {node_adjacencies[i]}"
...
if __name__ == "__main__":
    ...
    np.random.seed(42)
    sample_size = (
        5  # too big samples cause RecursionError but strangely not for my real datasets
    )
    df = pd.DataFrame(
        {
