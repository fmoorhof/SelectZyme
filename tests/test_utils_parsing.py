...
def test_parse_args_valid_config():
    ...
    with patch('builtins.open', mock_open(read_data=config_content)):
        with patch('sys.argv', ['utils.py', '--config', 'test_config.yml']):
            ...
            assert config['project']['data']['query_terms'] == ["ec:1.13.11.85", "ec:1.13.11.84"]
            assert config['project']['data']['length'] == "200 TO 601"
            assert config['project']['data']['custom_data_location'] == "/raid/data/fmoorhof/PhD/SideShit/LCP/custom_seqs_no_signals.csv"
            assert config['project']['data']['out_dir'] == "datasets/output/"
            assert config['project']['data']['df_coi'] == ["accession", "reviewed", "ec", "organism_id", "length", "xref_brenda", "xref_pdb", "sequence"]
