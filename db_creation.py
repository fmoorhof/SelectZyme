...
from qdrant_client import models, QdrantClient  # ! pip install qdrant-client

from minimal import read_fasta, gen_embedding
...
def filter_sequences(df: pd.DataFrame) -> pd.DataFrame:
    ...
    logging.info(f'{old_len-df.shape[0]} sequences were excluded because of exaggerated size (>=1024 amino acids)')
...
if collection_name not in str(collections_info):  # todo: implement this nicely: access the 'name' field of the object
    logging.info(f"Vector DB doesnt exist yet. Your .fasta will be embedded and a vector DB created under path=Vector_db/")
    ...
    # embed 
    embeddings = gen_embedding(df['Sequence'].tolist(), device='cuda')  # fails if sequences >= 1024 aa
...
logging.info(f"Retrieving data from Qdrant vector DB. This may take a while for some 100k sequences.")
...
logging.info(f"Starting 2D UMAP projection. This may take a while for some 100k sequences.")
...
df.loc[df['EC number'].str.contains('    '), 'EC number'] = '0.0.0.0'  # convert empty ECs to an artificial new EC class
cols = df.columns.values.tolist()
cols = cols[0:-3]
fig = px.scatter(df, x='x', y='y', color='EC number',
                 title='UMAP in 2D', 
                 hover_data=cols,
...
fig = px.histogram(cluster_counts, x='cluster', y='count', title='Cluster Frequencies',
                   labels={'cluster': 'Cluster', 'count': 'Frequency'},
                   barmode='overlay')  # Set barmode to 'overlay' or 'group')
